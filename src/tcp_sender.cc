...
{ 
  ...
  if (receive_window_ == 0){
    ...
    while ((receive_window_ > 0 && !stream_bytes.empty()) || (receive_window_ > 0 && writer().is_closed() && !fin_set_)){
      if (!syn_set_){
        syn_set_ = true;
        message.SYN = true;
        current_RTO_ = initial_RTO_ms_;
      }
      uint64_t available_payload_size = min({static_cast<uint64_t>(receive_window_- message.SYN), stream_bytes.size(), TCPConfig::MAX_PAYLOAD_SIZE});
...
{ 
  if(!send_buffer_.empty() && prev(send_buffer_.end())->first + prev(send_buffer_.end())->second.sequence_length() < msg.ackno) return;

  for (auto x = send_buffer_.begin(); x != send_buffer_.end(); x++){
    if (x->second.seqno + x->second.sequence_length() <= msg.ackno){
  
      send_buffer_.erase(x->first);
    }
  }
  current_RTO_ = initial_RTO_ms_;
  receive_window_ = msg.window_size;
  num_consecutive_retran_ = 0;

  if (pre_ackno_ < *msg.ackno){
    timer_ = 0;
    pre_ackno_ = *msg.ackno;
